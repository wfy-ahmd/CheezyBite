...
const SmartCart = () => {
    const { cart, cartTotal, itemAmount } = useContext(CartContext);
    ...
    // If cart is empty and not expanded, we can show a minimal "Empty" pill or hide it.
    ...
    return (
        <>
            ...
            <div
                ...
                className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-300 ${isExpanded ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
            />
            ...
            ${isExpanded
                        ...
                        : 'bottom-4 left-4 right-4 lg:left-auto lg:right-8 lg:w-auto h-16 rounded-full cursor-pointer hover:scale-105 active:scale-95'
                    }
            bg-charcoalBlack border border-white/10 shadow-2xl overflow-hidden
        `}
            ...
            >
                ...
                {/* === COLLAPSED PILL MODE === */}
                {/* Only visible when NOT expanded, using absolute positioning to fade out/in */}
                <div
                    onClick={toggleCart}
                    className={`absolute inset-0 flex items-center justify-between px-6 transition-opacity duration-300 ${isExpanded ? 'opacity-0 pointer-events-none' : 'opacity-100'} ${isBouncing ? 'animate-pulse' : ''}`}
                >
                ...
                <div className={`flex flex-col h-full w-full transition-opacity duration-300 delay-100 ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
