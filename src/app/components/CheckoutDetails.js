...
const CheckoutDetails = ({ setModal }) => {
  ...
  const { activeOrder, isOrderModalOpen, createOrder, setIsOrderModalOpen } = useContext(OrderContext);
  ...
  const handlePlaceOrder = () => {
    ...
    // Create order and it will auto-manage its own state
  ...
  // If there's an active order, show the timeline
  if (activeOrder && isOrderModalOpen) {
    return (
      <OrderTimeline
        order={activeOrder}
    ...
    <div className="lg:gap-x-8 h-full lg:px-12 lg:py-8">
      ...
      <div className="h-[86vh] lg:h-[47.5vh] flex flex-col lg:flex-row lg:gap-x-4">
        ...
        <div className="flex-1 h-full lg:max-w-[40%] flex flex-col justify-between pt-3 px-8 lg:p-0">
          ...
          <button onClick={handlePlaceOrder} className="btn btn-lg gradient w-full">
            Place order
          </button>
        </div>
      </div>
